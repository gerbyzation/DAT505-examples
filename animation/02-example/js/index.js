...
let currentValue = { x: 200, y: 0, z: 0 };
let leftPosition = { x: -200, y: 0, z: 0 }
let rightPosition = { x: 200, y: 0, z: 0 }

const toLeft = new TWEEN.Tween(currentValue).to(leftPosition, 1000);
const toRight = new TWEEN.Tween(currentValue).to(rightPosition, 1000);
...
function init(){
  ...
  window.addEventListener('click', function () {
    if (currentValue.x == 200) {
      toLeft.start()
    } else if (currentValue.x == -200) {
      toRight.start();
    }
  })
}
...
function animate(time){
  ...
  octoMain.position.x = currentValue.x;
  octoMain.position.y = currentValue.y;
  octoMain.position.z = currentValue.z;
